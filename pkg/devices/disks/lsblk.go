...
func ListBlockDevices(chroot string) (map[string]BlockDevice, error) {
	columns := []string{
		"KNAME",      // kernel name
		"SIZE",       // size
	...
	for s.Scan() {
		...
		for _, pair := range pairs {
			...
			case "MOUNTPOINT":
				mp := pair[2]
				if strings.HasPrefix(mp, chroot) {
					mp = strings.TrimPrefix(mp, chroot)
					if mp == "" {
						mp = "/"
					}
				}
			...
			case "MAJ:MIN":
				majorMinor = pair[2]
			default:
				klog.Infof("unexpected field from lsblk: %q", pair[1])
			}
		}
		...
		case typeLVM:
			...
			if strings.HasPrefix(majorMinor, "2:") {
				klog.Infof("ignoring flopping disk device: %+v", dev)
				continue
			}
		default:
		...
		if os.IsNotExist(err) {
			...
			klog.Infof("could not check if %q is in use: %v", dev.DeviceName, err)
...
func addHardwareInfo(dev *BlockDevice) error {
	klog.Infof(`executing "udevadm info" for %s`, dev.DeviceName)
	output, err := exec.Command(
		"udevadm", "info",
		"-q", "property",
		"--name", dev.DeviceName,
	...
	for s.Scan() {
		...
		if sep == -1 {
			klog.Infof("unexpected udevadm output line: %q", line)
			continue
		}
